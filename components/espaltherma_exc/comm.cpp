...
static void logBuffer(unsigned char *buffer, size_t len)
{
  char bufflog[250] = {0};
  ...
  ESP_LOGD(TAG, bufflog);
}

static int get_reply_len(char regID, char protocol='I')
{
  if (protocol == 'I')
  {
...
bool queryRegistry(char regID, unsigned char *buffer, char protocol)
{
  ...
  ESP_LOGD(TAG, "Querying register 0x%02x... ", (int)regID);
  ...
  while ((len < replyLen) && (millis() < (start + SER_TIMEOUT)))
  {
    if (MySerial.available())
    {
      ...
      if (len == 2 && buffer[0] == 0x15 && buffer[1] == 0xea)
      {
        ...
        ESP_LOGE(TAG, "Error 0x15 0xEA returned from HP\n");
  ...
  if (millis() >= (start + SER_TIMEOUT))
  {
    if (len == 0)
    {
      ESP_LOGE(TAG, "Time out! Check connection\n");
    }
    else
    {
      ESP_LOGE(TAG, "ERR: Time out on register 0x%02x! got %d/%d bytes\n", regID, len, replyLen);
  ...
  {
    ESP_LOGE(TAG, "ERROR: Wrong CRC on register 0x%02x. Calculated 0x%2x but got 0x%2x\nBuffer: ",regID, getCRC(buffer, len - 1), buffer[len - 1]);
    logBuffer(buffer,len);
    return false;
  }
  else
