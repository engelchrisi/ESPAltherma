...
{
public:
    void getLabels(LabelDef labelDefs[], size_t numLabels,
                    char registryID, LabelDef *ret[], int &num)
    {
        num = 0;
        for (int i= 0; i< numLabels; ++i) // auto &&label : labelDefs)
        {
            LabelDef& label= labelDefs[i];
    ...
    void readRegistryValues(LabelDef labelDefs[], size_t numLabels,
                            unsigned char *data, unsigned char protocol)
    {
        if (protocol == 'S')
        {
            // Registry ID is first byte
            readRegistryValues(labelDefs, numLabels,
                  data[0], data, 1);
        }
        else
        {
    ...
    void readRegistryValues(LabelDef labelDefs[], size_t numLabels,
                            char registryID, unsigned char *data, unsigned int offset)
    {
        // Serial.printf("For registry %d, we have these labels:\n", registryID);
        int num = 0;
        LabelDef *labels[128];
        getLabels(labelDefs, numLabels,
                  registryID, labels, num);

        if (num > _countof(labels))
        {
